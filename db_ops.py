...
def add_csv_to_data(user_name, passwd, file_name):
    if get_authority(user_name, passwd) == "super":
        client.database.data.delete_many({})
            ...
            for cat in listdir(model_dir):
                ...
                for img in listdir(cat_dir):
                    ...
                    if 'thumbnail' not in img:
                        images.append(get_thumbnail_name(join(prefix, join(cat, img))))
...
def get_images(user_name, passwd, model_id):
    if get_authority(user_name, passwd) == "super":
        item = client.database.images.find_one({"model_id" == model})
